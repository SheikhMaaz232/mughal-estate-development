...
@section('content')
    <div class="block block-rounded col-md-12">
        <div class="block-header block-header-default">
            <h3 class="block-title">@lang('messages.create-boq')</h3>
        </div>
        <div class="block-content block-content-full">
            <form action="{{ route('boq-masters.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control"
                            value="{{ App::getLocale() === 'ur' ? $tender->constructionSite->name_ur : $tender->constructionSite->name_en }}"
                            disabled>
                        <input type="hidden" name="construction_site_id" value="{{ $tender->construction_site_id }}">
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="tender_id" class="form-label">@lang('messages.tender')</label>
                        <input type="text" class="form-control"
                            value="{{ App::getLocale() === 'ur' ? $tender->title_ur : $tender->title_en }}" disabled>
                    ...
                    <div class="table-responsive">
                        <table class="table item-table table-bordered">
                            ...
                            <tbody>
                                ...
                                @if (count($oldItems) > 0)
                                    @foreach ($oldItems as $index => $itemId)
                                        <tr>
                                            ...
                                            <td>
                                                <input type="number" name="quantity[]"
                                                    ...
                                                    value="{{ old('quantity.' . $index) }}" step="0.0001" min="0">
                                            ...
                                            <td>
                                                <input type="number" name="rate[]"
                                                    ...
                                                    value="{{ old('rate.' . $index) }}" step="0.01" min="0">
                                                @error('rate.' . $index)
                                                    <div class="text-danger">{{ $message }}</div>
                                                @enderror
                                            </td>
                                            ...
                                            <td>
                                                <input type="number" name="gross_amount[]"
                                                    ...
                                                    value="{{ old('gross_amount.' . $index) }}" step="0.01">
                                            </td>
                                        </tr>
                                    @endforeach
                                @endif
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <input type="number" name="total_amount" id="total_amount"
                                    ...
                                    placeholder="@lang('messages.total_amount')" step="0.01" value="{{ old('total_amount', 0) }}"
                                    readonly>
                                @error('total_amount')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
    ...
    <script>
        ...
        let currentIndex = {{ count(old('item_id', [])) }};
