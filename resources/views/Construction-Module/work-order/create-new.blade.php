...
@section('content')
    <div class="block block-rounded col-md-12">
        <div class="block-header block-header-default">
            <h3 class="block-title">@lang('messages.create-work-order')</h3>
        </div>
            ...
            <form action="{{ route('work-orders.store') }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" value="{{ App::getLocale() === 'ur' ? $boqMaster->constructionSite->name_ur : $boqMaster->constructionSite->name_en }}" disabled>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" value="{{ App::getLocale() === 'ur' ? $boqMaster->tender->title_ur : $boqMaster->tender->title_en }}" disabled>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" value="{{ App::getLocale() === 'ur' ? $boqMaster->title_ur : $boqMaster->title_en }}" disabled>
                        <input type="hidden" name="boq_id" value="{{ $boqMaster->id }}">
                    </div>

                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold">@lang('messages.total_amount')</label>
                        <input type="text" class="form-control" value="{{ number_format($boqMaster->total_amount, 2) }}" disabled>
                    </div>
                </div>
                    ...
                    <div class="block-content pt-3 border-top">
                        <div class="row">
                            <div class="col-md-6 text-end">
                                <h5>@lang('messages.total_amount'): <span id="totalAmount" class="text-primary fw-bold">0.00</span></h5>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select id="itemSelect" class="form-select">
                            ...
                            @foreach($availableItems as $item)
                                <option value="{{ $item['id'] }}" data-rate="{{ $item['rate'] }}" data-quantity="{{ $item['remaining_quantity'] }}">
                                    {{ App::getLocale() === 'ur' ? ($item['item_name_ur'] ?? $item['item_name_en']) : $item['item_name_en'] }} (@lang('messages.remaining'): {{ $item['remaining_quantity'] }} {{ $item['unit'] }})
                                </option>
                            @endforeach
                        </select>
                    </div>
