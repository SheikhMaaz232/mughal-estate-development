...
@section('content')
    <div class="block block-rounded col-md-12">
        <div class="block-header block-header-default">
            <h3 class="block-title">@lang('messages.edit-detail-accounts')</h3>
        </div>
        <div class="block-content block-content-full">

            <form action="{{ route('detail-accounts.update', $detailAccount->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-lg-4">
                    ...
                    <div class="col-lg-8 col-xl-5">
                            ...
                            <select name="main_head_id" id="main-head"
                                ...
                                @foreach ($mainHeads as $mainHead)
                                    <option value="{{ $mainHead->id }}"
                                        {{ isset($detailAccount) && $detailAccount->main_head_id == $mainHead->id ? 'selected' : '' }}>
                            ...
                            <select name="control_head_id" id="control-head"
                                ...
                                @foreach ($controlHeads as $controlHead)
                                    <option value="{{ $controlHead->id }}"
                                        {{ isset($detailAccount) && $detailAccount->control_head_id == $controlHead->id ? 'selected' : '' }}>
                            ...
                            <select name="sub_head_id" id="sub-head"
                                ...
                                @foreach ($subHeads as $subHead)
                                    <option value="{{ $subHead->id }}"
                                        {{ isset($detailAccount) && $detailAccount->sub_head_id == $subHead->id ? 'selected' : '' }}>
                            ...
                            <select name="sub_sub_head_id" id="sub-sub-head"
                                ...
                                @foreach ($subSubHeads as $subSubHead)
                                    <option value="{{ $subSubHead->id }}"
                                        {{ isset($detailAccount) && $detailAccount->sub_sub_head_id == $subSubHead->id ? 'selected' : '' }}>
                            ...
                            <select name="sub_sub_sub_head_id" id="sub-sub-sub-head"
                                ...
                                @foreach ($subSubSubHeads as $subSubSubHead)
                                    <option value="{{ $subSubSubHead->id }}"
                                        {{ isset($detailAccount) && $detailAccount->sub_sub_sub_head_id == $subSubSubHead->id ? 'selected' : '' }}>
                            ...
                            <select name="party_id" id="party_id"
                                ...
                                @foreach ($searchParties as $searchParty)
                                    <option value="{{ $searchParty->id }}"
                                        {{ (old('searchParty') ?? $detailAccount->party_id) == $searchParty->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" class="form-control" id="name_en" name="name_en"
                                value="{{ old('name_en', $detailAccount->name_en) }}" required autocomplete="off">
                            @error('name_en')
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" class="form-control input-urdu" id="name_ur" name="name_ur"
                                value="{{ old('name_ur', $detailAccount->name_ur) }}" required autocomplete="off">
                            @error('name_ur')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <a href="{{ route('detail-accounts.index') }}" class="btn btn-secondary">@lang('messages.go-to-list')</a>
                    </div>
                </div>
            </form>
        </div>
