...
@section('content')
    <div class="block block-rounded col-md-12">
        ...
        <div class="block-content block-content-full">
            <form id="jvForm" action="{{ route('jv-voucher.update', $journalVoucher->id) }}" method="POST"
                class="row g-3">
                @csrf
                @method('PUT')
                ...
                <div class="col-md-12">
                    ...
                    <textarea type="text" name="description" class="form-control form-control-sm">{{ $journalVoucher->description ?? '' }}</textarea>
                </div>
                {{-- </div> --}}
                ...
                <div class="tab-content" id="pills-tabContent">
                    <div class="invoice-detail-items" style="padding: 0px 0px 0px 0px;">
                                ...
                                <tbody>
                                    @foreach ($journalVoucherDetails as $detail)
                                        ...
                                        <tr>
                                            ...
                                            <td hidden><input type="text" name="row_id[]" class="row_id"
                                                    value="{{ $index }}" hidden></td>

                                            <td>
                                                <select name="debit_detail_account_id[]" class="form-control select2">
                                                    <option value="">@lang('messages.select_debit')</option>
                                                    @foreach ($detailAccounts as $account)
                                                        <option value="{{ $account->id }}"
                                                            {{ (old('debit_detail_account_id') ?? $detail->debit_detail_account_id) == $account->id ? 'selected' : '' }}>
                                            ...
                                            <td>
                                                <select name="credit_detail_account_id[]" class="form-control select2">
                                                    <option value="">@lang('messages.select_credit')</option>
                                                    @foreach ($detailAccounts as $account)
                                                        <option value="{{ $account->id }}"
                                                            {{ (old('credit_detail_account_id') ?? $detail->credit_detail_account_id) == $account->id ? 'selected' : '' }}>
                                                            {{ App::getLocale() === 'ur' ? $account->name_ur : $account->name_en }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td>
                                                <input type="number" name="debit[]" value="{{ $detail->debit }}"
                                                    class="form-control debit">
                                            </td>

                                            <td>
                                                <input type="number" name="credit[]" value="{{ $detail->credit }}"
                                        ...
                                        <tr>
                                            <td></td>
                                            <td>
                                                <textarea name="detail_description_en[]" class="form-control">{{ $detail->detail_description_en }}</textarea>
                                            </td>
                                            <td>
                                                <textarea name="detail_description_ur[]" class="form-control">{{ $detail->detail_description_ur }}</textarea>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <a href="javascript:void(0);" class="btn btn-dark additem mt-3"
                            id="add-item">@lang('messages.add_details')</a>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" id="total_debit"
                                        value="{{ $journalVoucher->total_debit ?? '' }}"
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" id="total_credit"
                                        value="{{ $journalVoucher->total_credit ?? '' }}"
                ...
                <div class="col-xl-12 ">
                    <a href="{{ route('jv-voucher.index') }}" style="float: right;"
                        class="btn btn-dark rounded bs-popover ml-2 mt-5  mb-4">@lang('messages.go-to-list')</a>
    ...
    <script>
        document.getElementsByClassName('additem')[0].addEventListener('click', function() {
            ...
            let $html = '<tr>' +
                ...
                '<td class="text-right qty"> <input id="debit" type="number" name="debit[]"  placeholder="@lang('messages.debit')" class="form-control form-control-sm debit_' +
                currentIndex +
                '"></td>' +
                '<td class="text-right qty"> <input id="credit" type="number" name="credit[]" placeholder="@lang('messages.credit') " class="form-control form-control-sm credit_' +
                currentIndex +
                '"></td>' +
                '</td>' +
