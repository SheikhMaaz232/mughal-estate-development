    ...
    @section('content')
        <div class="block block-rounded col-md-12">
            <div class="block-header block-header-default">
                <h3 class="block-title">@lang('messages.crv')</h3>
            </div>
            <div class="block-content block-content-full">
                <form action="{{ route('cash-receipt-voucher.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="date" class="form-label">@lang('messages.voucher_no')</label>

                            <input class="form-control" name="date" value="CRV-{{ $maxid }}" disabled>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="detail_account_id" id="detail_account_id"
                                ...
                                @foreach ($coaReceivables as $coaReceivable)
                                    <option value="{{ $coaReceivable->id }}"
                                        {{ old('detail_account_id') == $coaReceivable->id ? 'selected' : '' }}>
                            ...
                            <select name="cash_account_id" id="cash_account_id"
                                ...
                                @foreach ($coaCashAccounts as $coaCashAccount)
                                    <option value="{{ $coaCashAccount->id }}"
                                        {{ old('cash_account_id') == $coaCashAccount->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="transaction_type" id="transaction_type"
                                ...
                                <option value="registry_fees"
                                    {{ old('transaction_type') == 'registry_fees' ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <a href="{{ route('bank-receipt-voucher.index') }}"
                                class="btn btn-dark">@lang('messages.go-to-list')</a>
                        </div>
                    </div>
