...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="card-footer">
            <a href="{{ route('relations.edit', $city->id) }}" class="btn btn-warning">Edit</a>
            <form action="{{ route('relations.destroy', $city->id) }}" method="POST" style="display:inline;">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger" onclick="return confirm('Are you sure?')">Delete</button>
            </form>
            <a href="{{ route('relations.index') }}" class="btn btn-secondary">Back to List</a>
        </div>
    </div>
</div>
@endsection
