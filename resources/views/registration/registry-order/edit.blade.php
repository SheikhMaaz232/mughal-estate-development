    ...
    <div class="container-fluid">
        <div class="card custom-card">
            <div class="card-header">
                <h4 class="card-title">{{ __('messages.edit_registry_letter') }}</h4>
            </div>

            <form id="registry-order-form" action="{{ route('registry-order.update', $registryOrder->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="date"
                                    value="{{ old(
                                        'date',
                                        isset($registryOrder) && $registryOrder->date
                                            ? \Carbon\Carbon::parse($registryOrder->date)->format('Y-m-d')
                                            : now()->format('Y-m-d'),
                                    ) }}">
                                @error('date')
                            ...
                            <select name="party_id" id="party_id"
                                ...
                                @foreach ($searchParties as $searchParty)
                                    <option value="{{ $searchParty->id }}"
                                        {{ old('party_id', $registryOrder->party_id) == $searchParty->id ? 'selected' : '' }}>
                        ...
                        <input type="hidden" name="booking_id" value="{{ $registryOrder->booking_id }}">
                    </div>
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select name="relation" class="form-control">
                                    ...
                                    <option value="file_owner"
                                        {{ old('relation', @$registryOrder->relation ?? '') == 'file_owner' ? 'selected' : '' }}>
                                        {{ __('messages.file_owner') }}
                                    </option>

                                    <option value="nominee"
                                        {{ old('relation', @$registryOrder->relation ?? '') == 'nominee' ? 'selected' : '' }}>
                                        {{ __('messages.nominee') }}
                                    </option>

                                    <option value="blood_relation"
                                        {{ old('relation', @$registryOrder->relation ?? '') == 'blood_relation' ? 'selected' : '' }}>
                                        {{ __('messages.blood_relation') }}
                                    </option>

                                    <option value="third_party"
                                        {{ old('relation', @$registryOrder->relation ?? '') == 'third_party' ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>{{ __('messages.fard_id') }}</label>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control" id="registry_fees" name="registry_fees"
                                step="any" min="0" onwheel="this.blur()" value="{{ old('registry_fees', @$registryOrder->registry_fees ?? 0) }}"
                            ...
                            <select name="registry_fees_receivable_account" id="registry_fees_receivable_account_id"
                                ...
                                @foreach ($registryAccounts as $registryAccount)
                                    <option value="{{ $registryAccount->id }}"
                                        {{ old('registry_fees_receivable_account', $registryOrder->registry_fees_receivable_account ?? '') == $registryAccount->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control form-select registry_status-dropdown" id="registry_status"
                                ...
                                <option value="underprocess"
                                    {{ old('registry_status', @$registryOrder->registry_status ?? '') === 'underprocess' ? 'selected' : '' }}>
                                    @lang('messages.underprocessRegistry')
                                </option>
                                <option value="completed"
                                    {{ old('registry_status', @$registryOrder->registry_status ?? '') === 'completed' ? 'selected' : '' }}>
                                    @lang('messages.completedRegistry')
                                </option>
                                <option value="pending"
                                    {{ old('registry_status', @$registryOrder->registry_status ?? '') === 'pending' ? 'selected' : '' }}>
