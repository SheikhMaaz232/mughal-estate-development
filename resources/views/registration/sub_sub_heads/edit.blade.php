...
@section('content')
    <div class="block block-rounded col-md-12">
        <div class="block-header block-header-default">
            <h3 class="block-title">@lang('messages.edit-sub-sub-heads')</h3>
        </div>
        <div class="block-content block-content-full">

            <form action="{{ route('sub-sub-heads.update', $subSubHead->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-lg-4">
                    ...
                    <div class="col-lg-8 col-xl-5">
                            ...
                            <select name="main_head_id" id="main-head"
                                ...
                                @foreach ($mainHeads as $mainHead)
                                    <option value="{{ $mainHead->id }}"
                                        {{ isset($subSubHead) && $subSubHead->main_head_id == $mainHead->id ? 'selected' : '' }}>
                            ...
                            <select name="control_head_id" id="control-head"
                                ...
                                @foreach ($controlHeads as $controlHead)
                                    <option value="{{ $controlHead->id }}"
                                        {{ isset($subSubHead) && $subSubHead->control_head_id == $controlHead->id ? 'selected' : '' }}>
                            ...
                            <select name="sub_head_id" id="sub-head"
                                ...
                                @foreach ($subHeads as $subHead)
                                    <option value="{{ $subHead->id }}"
                                        {{ isset($subSubHead) && $subSubHead->sub_head_id == $subHead->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="name_en" name="name_en"
                                value="{{ old('name_en', $subSubHead->name_en) }}" required autocomplete="off">
                            @error('name_en')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control input-urdu" id="name_ur" name="name_ur"
                                value="{{ old('name_ur', $subSubHead->name_ur) }}" required autocomplete="off">
                            @error('name_ur')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <a href="{{ route('sub-sub-heads.index') }}" class="btn btn-secondary">@lang('messages.go-to-list')</a>
                    </div>
                </div>
            </form>
        </div>
