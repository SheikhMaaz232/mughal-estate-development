    ...
    <div class="content content-full">
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
            <div class="flex-grow-1">
                <h1 class="h3 fw-bold mb-4">@lang('messages.tehsil-management')</h1>
                <h2 class="fs-base lh-base fw-medium text-muted mb-0">@lang('messages.list-of-all-tehsils')</h2>
            </div>
            <a href="{{ route('tehsils.create') }}" class="btn btn-sm btn-primary">@lang('messages.add-new')</a>
        ...
        <div class="block-content block-content-full">
            <table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                ...
                <tbody>
                    ...
                    <td>{{ $tehsil->id }}</td>
                    <td>{{ $tehsil->city->name_en }}</td>
                    <td>{{ $tehsil['name_en'] }}</td>
                    <td>{{ $tehsil['name_ur'] }}</td>
                    <td class="text-center">
                        <div class="btn-group">
                            <!-- Edit Button -->
                            <a href="{{ route('tehsils.edit', $tehsil->id) }}"
                            ...
                            <!-- Delete Form -->
                            <form method="POST" action="{{ route('tehsils.destroy', $tehsil->id) }}" class="d-inline-block delete-form">
                                @csrf
                                @method('DELETE')
