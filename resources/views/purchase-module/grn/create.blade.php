...
@section('content')
    <div class="block block-rounded col-md-12">
        ...
        <div class="block-content block-content-full">
            <form id="grn-form" action="{{ route('grn.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input class="form-control" name="purchase_order_no" value="{{ $purchaseOrderMaster->id }}"
                            readonly>
                    </div>
                ...
                <div class="row">
                        ...
                        <select name="project_id" id="project_id"
                            ...
                            @foreach ($projects as $project)
                                <option value="{{ $project->id }}"
                                    {{ (old('project_id') ?? $purchaseOrderMaster->project_id) == $project->id ? 'selected' : '' }}>
                                    {{ App::getLocale() === 'ur' ? $project->name_ur ?? '-' : $project->name_en ?? '-' }}
                                </option>
                            @endforeach
                        </select>
                        <input type="hidden" name="project_id" value="{{ $purchaseOrderMaster->project_id }}">
                        @error('project_id')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="party_id" id="party_id"
                            ...
                            @foreach ($searchParties as $searchParty)
                                <option value="{{ $searchParty->id }}"
                                    {{ (old('party_id') ?? $purchaseOrderMaster->party_id) == $searchParty->id ? 'selected' : '' }}>
                        ...
                        <input type="hidden" name="party_id" value="{{ $purchaseOrderMaster->party_id }}">
                        @error('party_id')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                        ...
                        <select name="detail_account_id" id="detail_account_id"
                            ...
                            @foreach ($detailAccounts as $detailAccount)
                                <option value="{{ $detailAccount->id }}"
                                    {{ (old('detail_account_id') ?? $purchaseOrderMaster->detail_account_id) == $detailAccount->id ? 'selected' : '' }}>
                                    {{ App::getLocale() === 'ur' ? $detailAccount->name_ur ?? '-' : $detailAccount->name_en ?? '-' }}
                                </option>
                            @endforeach
                        </select>
                        <input type="hidden" name="detail_account_id"
                            value="{{ $purchaseOrderMaster->detail_account_id }}">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="supplier_bill_no" name="supplier_bill_no"
                            placeholder="@lang('messages.supplier-bill-no')" value="{{ old('supplier_bill_no') }}">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="driver_name" name="driver_name"
                            placeholder="@lang('messages.driver_name')" value="{{ old('driver_name') }}">
                        @error('driver_name')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="fare" name="fare"
                            placeholder="@lang('messages.fare')" value="{{ old('fare') }}">
                        @error('fare')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="unloaded_by" name="unloaded_by"
                            placeholder="@lang('messages.unloaded_by')" value="{{ old('unloaded_by') }}">
                        @error('unloaded_by')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <textarea type="text" class="form-control" id="remarks" name="remarks" placeholder="@lang('messages.remarks')"
                            autocomplete="off" value="{{ old('remarks') }}">{{ old('remarks') }}</textarea>
                        @error('remarks')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                                            ...
                                            <tr>
                                                ...
                                                <td class="received_qty">
                                                    <input type="number" name="received_qty[]"
                                                        value="{{ $purchaseOrderDetail->received_qty }}" step="any"
                                                ...
                                                <td class="balance">
                                                    <input type="text" name="balance[]"
                                                        value="{{ $purchaseOrderDetail->balance }}"
