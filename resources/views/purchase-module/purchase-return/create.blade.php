...
@section('content')
    <div class="block block-rounded col-md-12">
        ...
        <div class="block-content block-content-full">
            <form id="purchase-return-form" action="{{ route('purchase-return.store') }}" method="POST">
                @csrf
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input class="form-control" style="background-color: #e9ecef !important;" name="purchase_invoice_no"
                            value="{{ $purchaseMasterData->id }}" readonly>
                    </div>
                    <div class="col-md-3 mb-3">
                        <label for="purchase_order_no" class="form-label">@lang('messages.grn_no')</label>

                        <input class="form-control" style="background-color: #e9ecef !important;" name="grn_no"
                            value="{{ $purchaseMasterData->grn_no }}" readonly>
                    </div>
                    <div class="col-md-3 mb-3">
                        ...
                        <input class="form-control" style="background-color: #e9ecef !important;" name="purchase_order_no"
                            value="{{ $purchaseMasterData->purchase_order_no }}" readonly>
                    </div>
                </div>
                <div class="row">
                        ...
                        <select name="project_id" id="project_id"
                            ...
                            @foreach ($projects as $project)
                                <option value="{{ $project->id }}"
                                    {{ (old('project_id') ?? $purchaseMasterData->project_id) == $project->id ? 'selected' : '' }}>
                                    {{ App::getLocale() === 'ur' ? $project->name_ur ?? '-' : $project->name_en ?? '-' }}
                                </option>
                            @endforeach
                        </select>
                        <input type="hidden" name="project_id" value="{{ $purchaseMasterData->project_id }}">
                        @error('project_id')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="party_id" id="party_id"
                            ...
                            @foreach ($searchParties as $searchParty)
                                <option value="{{ $searchParty->id }}"
                                    {{ (old('party_id') ?? $purchaseMasterData->party_id) == $searchParty->id ? 'selected' : '' }}>
                        ...
                        <input type="hidden" name="party_id" value="{{ $purchaseMasterData->party_id }}">
                        @error('party_id')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select name="detail_account_id" id="detail_account_id"
                            ...
                            @foreach ($detailAccounts as $detailAccount)
                                <option value="{{ $detailAccount->id }}"
                                    {{ (old('detail_account_id') ?? $purchaseMasterData->detail_account_id) == $detailAccount->id ? 'selected' : '' }}>
                                    {{ App::getLocale() === 'ur' ? $detailAccount->name_ur ?? '-' : $detailAccount->name_en ?? '-' }}
                                </option>
                            @endforeach
                        </select>
                        <input type="hidden" name="detail_account_id"
                            value="{{ $purchaseMasterData->detail_account_id }}">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" style="background-color: #e9ecef !important;" class="form-control"
                            ...
                            value="{{ old('supplier_bill_no', $purchaseMasterData->supplier_bill_no) }}" readonly>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" style="background-color: #e9ecef !important;" class="form-control"
                            ...
                            value="{{ old('unloaded_by', $purchaseMasterData->unloaded_by) }}" readonly>
                        @error('unloaded_by')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                            ...
                            <table class="table item-table">
                                ...
                                <tbody>
                                    ...
                                    @if (!empty($purchaseDetailsData))
                                        @foreach ($purchaseDetailsData as $purchaseDetailData)
                                            @php
                                                $index = $loop->index + 2;
                                            @endphp
                                            <tr>
                                                    ...
                                                    <select id="product_id"
                                                        ...
                                                        @foreach ($items as $productData)
                                                            <option value="{{ $productData->id }}"
                                                                {{ (old('product_id') ?? $purchaseDetailData->product_id) == $productData->id ? 'selected' : '' }}>
                                                                {{ App::getLocale() === 'ur' ? $productData->name_ur ?? '-' : $productData->name_en ?? '-' }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                    <input type="hidden" name="product_id[]"
                                                        value="{{ $purchaseDetailData->product_id }}">
                                                </td>
                                                ...
                                                <td class="quantity">
                                                    <input type="text" style="color: black;"
                                                        value="{{ old('quantity', $purchaseDetailData->quantity) }}"
                                                ...
                                                <td class="price">
                                                    <input type="number" name="price[]"
                                                        value="{{ $purchaseDetailData->price }}" step="any"
                ...
                <div class="row justify-content-end">
                    <div class="col-md-6 mb-3">
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input type="number" id="carriage" style="color: black;" name="carriage"
                                    value="{{ old('carriage') }}" step="any" min="0" onwheel="this.blur()"
                ...
                <div class="row justify-content-end">
                    <div class="col-md-6 mb-3">
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input type="number" id="tax" style="color: black;" name="tax"
                                    value="{{ old('tax') }}" step="any" min="0" onwheel="this.blur()"
                ...
                <div class="row justify-content-end">
                    <div class="col-md-6 mb-3">
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input type="number" id="other_amount" style="color: black;" name="other_amount"
                                    value="{{ old('other_amount') }}" step="any" min="0" onwheel="this.blur()"
