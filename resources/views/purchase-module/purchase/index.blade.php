        ...
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
                <div class="flex-grow-1">
                    <h2 class="fs-base lh-base fw-medium text-muted mb-0">@lang('messages.list-of-purchase-invoice')</h2>
                </div>
                <a href="{{ route('purchase-invoice.generate') }}" class="btn btn-sm btn-primary">@lang('messages.add-purchase-invoice')</a>
            </div>
        </div>
    </div>
        ...
        <form method="GET" action="{{ route('purchase-invoice.index') }}">
            ...
            <div class="row">
                <div class="col-lg-4 mb-3">
                    ...
                    <input type="number" class="form-control" name="id" placeholder="@lang('messages.purchase_invoice_no')" step="any"
            ...
            <div class="row">
                <div class="col-lg-6 mb-3">
                    ...
                    @if (request()->hasAny(['search', 'party_id', 'occupation_id', 'sub_head_id', 'sub_sub_head_id']))
                        <a href="{{ route('purchase-invoice.index') }}" class="btn btn-secondary">@lang('messages.clear')</a>
                    @endif
                </div>
            </div>
        </form>
        ...
        <div class="block block-rounded">
            <div class="block-content block-content-full">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @foreach ($purchaseInvoicesListing as $purchaseInvoiceListing)
                            <tr>
                                <td class="text-center">{{ $purchaseInvoiceListing->id }}</td>
                                <td class="text-center">
                                    @lang('messages.PO'){{ ' - ' . $purchaseInvoiceListing->purchase_order_no }}</td>
                                <td class="text-center">
                                    @lang('messages.GRN'){{ ' - ' . $purchaseInvoiceListing->grn_no }}</td>
                                ...
                                <td>{{ $purchaseInvoiceListing->net_amount }}</td>
                                <td>
                                ...
                                <td class="text-left">
                                    <div class="btn-group">
                                        @if ($purchaseInvoiceListing->status === 'Unverified')
                                            <form method="POST"
                                                action="{{ route('purchase-invoice.updateStatus', $purchaseInvoiceListing->id) }}"
                                        ...
                                        <a href="{{ route('purchase-invoice.edit', $purchaseInvoiceListing->id) }}"
                                        ...
                                        @if ($purchaseInvoiceListing->status === 'Unverified')
                                            <form method="POST"
                                                action="{{ route('purchase-invoice.destroy', $purchaseInvoiceListing->id) }}"
                                        ...
                                        @endif
                                        <a href="{{ route('purchase-invoice.show', $purchaseInvoiceListing->id) }}"
                                            class="btn btn-sm btn-alt-secondary js-bs-tooltip-enabled"
                                    ...
                                    <div class="mt-2">
                                        <form
                                            action="{{ route('purchase-return.create', ['id' => $purchaseInvoiceListing->id]) }}"
                                            method="GET">
                                            <input type="hidden" name="purchase_invoice_no"
                                                value="{{ $purchaseInvoiceListing->id }}">
                                            <button type="submit" class="action-btn btn-edit bs-tooltip me-2"
                                                data-toggle="tooltip" data-placement="top" title="Enter Invoice Info">
                                                <i class="fa-solid fa-file-minus"></i>
                                                @lang('messages.createPurchaseReturn')
                                            </button>
                                        </form>
                                    </div>
                                    {{-- @endif --}}
